...
## دریافت محدودیت‌های وکالت کاربر در بازار تعهدی

```shell
curl 'https://apiv2.nobitex.ir/margin/v2/delegation-limit?market=BTCUSDT' \
  -H "Authorization: Token yourTOKENhereHEX0000000000"
```

```plaintext
http GET https://apiv2.nobitex.ir/margin/v2/delegation-limit \
  market=BTCUSDT
```
...
برای دریافت سقف باقی‌مانده وکالت کاربر در یک بازار تعهدی از این درخواست استفاده کنید.
این نسخه جایگزین endpoint قدیمی `GET /margin/delegation-limit` شده است و به جای `currency` باید `market` ارسال شود.
در پاسخ، محدودیت‌های باقی‌مانده به تفکیک سمت `buy` و `sell` و برای هر اهرم مجاز بازار بازگردانده می‌شود.
در سمت `sell` واحد `limit` برابر رمزارز مبدا بازار و در سمت `buy` واحد `limit` برابر رمزارز مقصد بازار است.
...
| پارامتر | نوع    | پیش‌فرض | توضیحات                   | نمونه               |
|---------|--------|---------|---------------------------|---------------------|
| market  | string | الزامی  | نماد بازار تعهدی نوبیتکس | `BTCUSDT` یا `BTCIRT` |
...
|-------------|--------|------------------------------------------------------------------------------------|------------------------------------|
| status      | string | وضعیت پاسخ                                                                         | `ok`                               |
| limits      | object | محدودیت‌های باقی‌مانده وکالت به تفکیک سمت معامله                                   | `{"buy": [...], "sell": [...]}`    |
| limits.buy  | array  | محدودیت‌های خرید تعهدی در بازار؛ واحد `limit` در این بخش برابر رمزارز مقصد بازار است | `[{"leverage": "1", "limit": "1250"}]` |
| limits.sell | array  | محدودیت‌های فروش تعهدی در بازار؛ واحد `limit` در این بخش برابر رمزارز مبدا بازار است  | `[{"leverage": "1", "limit": "0.021"}]` |

### شی DelegationLimitItem

| پارامتر  | نوع     | توضیحات                                              | نمونه    |
|----------|---------|------------------------------------------------------|----------|
| leverage | decimal | اهرم مربوط به محدودیت باقی‌مانده                    | `1.5`    |
| limit    | decimal | سقف باقی‌مانده قابل استفاده در همان سمت و همان اهرم | `1875`   |
...
| InvalidMarket        | بازار درخواستی در نوبیتکس یافت نشد.                            |
| UnsupportedMarginSymbol | بازار درخواستی از معاملات تعهدی پشتیبانی نمی‌کند.           |

### نکات و ملاحظات
1. نمونه پاسخ بالا کوتاه‌شده است و آرایه هر سمت از اهرم `1` تا حداکثر اهرم مجاز آن بازار، با گام `0.5` ادامه پیدا می‌کند.
2. مقدار `limit` در هر ورودی، سقف باقی‌مانده قابل استفاده برای همان سمت و همان اهرم است.
3. با باز بودن موقعیت‌ها و سفارش‌های تعهدی، این مقادیر کاهش می‌یابند و با بستن یا تسویه آن‌ها دوباره افزایش پیدا می‌کنند.
4. در سفارش فروش تعهدی، مقدار سفارش باید از `sell.limit` اهرم انتخابی کمتر یا مساوی باشد.
5. در سفارش خرید تعهدی، ارزش کل سفارش (`price × amount`) باید از `buy.limit` اهرم انتخابی کمتر یا مساوی باشد.
