...
| clientOrderId       | string | `null` | شماره سفارش کاربر، تا ۸ کاراکتر،‌ یکتا  برای هر کاربر(آزمایشی)          | `order1`                                      |
...
3. `clientOrderId` .در حالت آزمایشی است و ممکن است در آینده تغییر کند
...
2. اگر هر دو ارسال شوند اولویت با `id` است.
3. `clientOrderId` .در حالت آزمایشی است و ممکن است در آینده تغییر کند
4. مقدار status میتواند از 'new' به 'active' و یا از 'active'/'inactive' به 'cancel' تغییر کند.
در غیر اینصورت، درخواست رد میشود.
5. در صورتی که سفارش درخواست شده جزئی از یک سفارش OCO انجام نشده باشد، هر دو سفارش مرتبط لغو خواهند شد.
