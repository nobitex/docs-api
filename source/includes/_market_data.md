...
<h2 id="orderbook-v3">لیست سفارش‌ها: اردربوک V3</h2>
...
این اندپوینت، به جز یک مورد، کاملاً مشابه [API ورژن ۲](#orderbook) می‌باشد با این تفاوت که جای کلید‌های `asks` و `bids` تغییر کرده‌است. در واقع در این ورژن استفاده از دو عبارت `bids` و `asks` نسبت به نسخه v2 اصلاح و جابجا شده است؛ بنابراین سفارش‌های **خرید** در **`bids`** و سفارش‌های **فروش** در **`asks`** بازگردانده می‌شوند.
