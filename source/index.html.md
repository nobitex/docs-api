...
* آدرس: `POST /users/wallets/withdraw-confirm`
...
```shell
curl -X POST 'https://api.nobitex.ir/users/wallets/withdraw-cancel' \
  -H 'Authorization: Token yourTOKENhereHEX0000000000' \
  -H 'Content-Type: application/json' \
  --data '{"withdraw": 432}'
```
...
```plaintext
http POST https://api.nobitex.ir/users/wallets/withdraw-cancel \
  withdraw=432
```
...
* آدرس: `POST /users/wallets/withdraw-cancel`
...
<aside class="notice">
محدودیت فراخوانی : 10 درخواست در دقیقه، 60 درخواست در ساعت
</aside>
...
```shell
curl -X POST 'https://api.nobitex.ir/withdraws/433/update-status' \
  -H 'Authorization: Token yourTOKENhereHEX0000000000'
```


```plaintext
http POST https://api.nobitex.ir/withdraws/433/update-status
```
...
برای به‌روزرسانی وضعیت یک درخواست برداشت از این نوع درخواست استفاده نمایید:

* آدرس: `POST /withdraws/WITHDRAW/update-status`
