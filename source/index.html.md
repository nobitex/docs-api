...
##آمار بازار نوبیتکس بر اساس زمان 

```shell
curl 'https://api.nobitex.net/market/udf/history?symbol=BTCIRT&resolution=D&from=1562120967&to=1562230967' 
```

```plaintext
http GET https://api.nobitex.net/market/udf/history?symbol=BTCIRT&resolution=D&from=1562120967&to=1562230967
```
...
برای دریافت  آمار بازار نوبیتکس  بر اساس زمان از این نوع درخواست استفاده نمایید:

- آدرس : `GET /market/udf/history?symbol={pair coin}&resolution={time resolution}&from={start timestamp}&to={end timestamp}`
...
resolution | string |   الزامی |بازه زمانی هر خروجی| `rls`
from | string |   الزامی |زمان ابتدای بازه| `1562120967`
to | string |   الزامی |زمان پایان بازه| `1562230967`

- پارامتر Resolution  بازه زمانی هر خروجی می‌باشد و مقدار آن می‌تواند یکی از مقادیر زیر باشد:
