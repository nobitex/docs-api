...
const entryMatchesQuery = (entry: SearchEntry, rawQuery: string): boolean => {
  ...
  const haystack = `${entry.pageTitle} ${entry.headingText} ${entry.content ?? ''}`
  const haystackTokens = textToTokens(haystack)
  if (!haystackTokens.length) return false

  return queryTokens.every((q) => haystackTokens.includes(q))
}
